...
class AuthService {
    ...
    signup(AuthDto, errCallback) {
        return __awaiter(this, void 0, void 0, function* () {
            const existingUser = yield this.userService.findOneByEmail(AuthDto.email);
            if (existingUser)
                return errCallback(new common_1.BadRequestError("user already exists with same email"));
    ...
    signin(signInDTO, errCallback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user)
                return errCallback(new common_1.BadRequestError("user doesn't exist with this email."));
            const samePwd = this.authenticationService.pwdCompare(user.password, signInDTO.password);
            if (!samePwd)
                return errCallback(new common_1.BadRequestError("wrong credentials"));
